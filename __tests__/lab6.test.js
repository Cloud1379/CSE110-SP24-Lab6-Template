describe('Basic user flow for Website', () => {
    ...
    it('Clicking the "Add to Cart" button should change button text', async () => {
        ...
        const queryItem = await page.waitForSelector('product-item')
        const shadowRoot = await queryItem.getProperty('shadowRoot');
    ...
    it('Checking number of items in cart on screen', async () => {
        ...
        const queryItems = await page.$$('product-item');
        for (let i = 1; i < queryItems.length; i++) {
            const shadowRoot = await queryItems[i].evaluateHandle(item => item.shadowRoot);
    ...
    it('Checking number of items in cart on screen after reload', async () => {
        ...
        for (const productItem of productItems) {
            ...
            const cartButton = await shadowRoot.$('button');
            const buttonData = await page.evaluate(element => element.innerText, cartButton);
            expect(buttonData).toBe('Add to Cart');
        }
        const cartCount = await page.$eval('#cart-count', element => parseInt(element.innerText));
        expect(cartCount).toBe(0);
